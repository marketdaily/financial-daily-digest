...
RULES = [
    ...
    # ── 內容誠實(法務部檢查表 §2) ──────────────────────────────────────
    {
        ...
        # ⚠️ 2026-09-15:本條自 09-08 起在 md_docs_rest 天天紅 2 面 HIGH,而命中的是
        # 日報正文**引用的新聞標題**:「今日新聞點名『聯電散戶突破百萬人,矽晶圓三雄…』」。
        # 「突破百萬人」配到 (已有|超過|突破|逾)…(人) —— 但那是第三方媒體對聯電散戶人數的
        # 報導,不是我們對自己績效/規模的表示,公平法 §21 管的是後者。
        # HIGH 級法務誤告最貴的地方不是多一則告警,是它教人忽略這個通道。
        # 豁免刻意寫窄:只有【新聞/報導/媒體/外電/標題】引介的引號內文才遮罩,
        # 想用它藏自家宣稱,得先在前面寫「新聞點名」四個字。引號外一個字都不遮。
        "exempt": [r"(新聞|報導|媒體|外電|標題)[^「\n]{0,10}「[^」\n]{0,120}」"],
        "negatives": [
            "方向勝率與避坑勝率全部公開,逐筆可對照紀錄表。",
            "勝率不等於獲利。一個 80% 的東西也可能虧錢。",
            "沒有任何人能保證勝率,我們把錯的也列出來。",
            "今日新聞點名「聯電散戶突破百萬人,矽晶圓三雄台勝科攻漲停」,需留意散戶過熱追高風險。",
        ],
    },
    {
