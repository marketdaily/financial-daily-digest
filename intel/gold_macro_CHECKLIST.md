...
骨架已產生(gold_macro.py + capabilities/tests/gold_macro.test.sh),以下才是真正的工作。
...
- [ ] `intel/patrol.py` 開頭 import 加上 `gold_macro`
- [ ] `patrol.py::run()` 內加一段呼叫 `gold_macro.scan(watchlist_codes)`,結果併入 by_code
      (同碼多筆訊號合併時優先序 red > yellow > plain,別讓後面迴圈把更嚴重的蓋掉)
- [ ] 若不受 watchlist codes 限制(如 tw_fsc.py 自成一套金融機構清單),自成一組 loop,別硬塞進 codes 迴圈

## 收尾(mission 規定,缺一不可)
- [ ] 填完 `capabilities/tests/gold_macro.test.sh` 的 TODO,`bash capabilities/tests/gold_macro.test.sh` 過
