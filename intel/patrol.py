...
from intel import tw_institutional, mops_watch, us_analyst, us_insider, us_8k_events, tw_margin, tw_sbl, tw_holders, tw_investor_conf, tw_investor_materials, tw_surveillance, tw_fsc, us_sec_regulatory, news_signals, signal_ledger, us_13f_ledger, tw_financials, tw_leadflow, tw_analyst
...
def run():
    ...
    try:
        ...
        leadflow_data = {}
    try:
        analyst_tw = tw_analyst.scan(wl)  # 台股券商評等/目標價調整,標題自帶代碼=全市場,不受 codes 過濾
    except Exception as e:
        print(f"tw_analyst 掃描失敗(不擋巡邏):{e}")
        analyst_tw = {}
    ...
    for c, f10 in analyst_tw.items():
        _emit(c, f10["level"], f10["signal"], "tw_analyst")
    materials_fetched = 0
    for c in codes:
        f2 = conf_data.get(c)
