...
- **🤖 cron 呼叫 `claude -p` 一律指定 `--model opus --fallback-model sonnet`(2026-07-30 Delvin 親令「這種都用 opus 就好」)**：不吃 CLI 預設(=Fable 5,週額度見底整批 cron 同時失敗刷屏——07-30 news_reactive 每 10 分鐘推一次告警即此因)。現役:news_reactive(Python 端模型鏈 opus→sonnet→haiku)、site_scan / digest_selfheal / marketing_agents_weekly / line_group;自主機器 `~/autonomous/config.sh` 本來就 opus。**新增任何跑 claude 的排程都要照辦**。
