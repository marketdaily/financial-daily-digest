...
- **內容/創作**：content-engine、crosspost、content-repurposing、content-refresh、content-research-writer、brand-voice-amplifier（⚠️enhancer 退役）、podcast-outline、video-style、video-editing、story-script、youtube-summarizer、video-watcher（真的看影片:下載+抽格視覺讀+timeline beats,IG牆有Playwright fallback）、article-extractor
...
- **🧠 記憶單機主寫制 → B 級：副機大腦唯讀（2026-07-30 Delvin 拍板升級）**：winrig=唯一寫者。**Mac 可以「新增」記憶 topic 檔（會正常送出），但不能「修改」既有記憶/skill 檔**——sync.sh 副機階段1 用 `--ignore-existing`，寫入權由程式收掉，不靠自律；在 Mac 改既有檔不會傳出去，只會被存證到 `~/.brain-local-edits` 並由 `brain_sync_deadman.sh` 推播。要改既有記憶/skill 就經 winrig MCP 在 winrig 端改。新增記憶仍需登記 `MEMORY_INBOX_MAC.md`（winrig 每 2h `scripts/memory_inbox_pull.sh` SSH 拉走；該檔已排除在 sync 之外，兩機不互相覆蓋）。winrig 端改索引只准 Edit 錨定，禁整檔 Write。詳見 memory `feedback_memory_single_writer`、`project_brain_sync_realtime`。
