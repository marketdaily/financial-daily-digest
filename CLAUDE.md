...
- **名稱：不改名，沿用 MarketDaily／@marketdailyhq／marketdaily.ai（2026-09-14 老闆拍板「just keep using marketdaily ai, its fine」）**。理由：Threads @marketdailyhq 是**活的**（每日 2,412 views、13 追蹤者），開新帳號等於丟掉一個已在運作的分發管道從零開始。⚠️ 線上 Threads 簡介仍寫著「75+ 來源」（2026-05-26 即標記為捏造的數字），新 bio 已備在 `brand.json.profile`，**但 Threads 簡介 API 不開放寫入，需老闆本人在 App 改**。
- **⭐⭐ 本內容線改自動發（2026-09-15 老闆授權，原話「你覺得好的都發可以一篇發好幾篇文也不會怎麼樣」）**：`run post --auto` 產完即發，**不再逐則等核可**。⚠️ 這是老闆本人推翻自己 08-17 的「對外發布一律先給老闆看」，**僅限 newsroom 這條線**——日報寄信、行銷素材、對外提案仍照舊規，不要擴張解釋。品質判準交給閘門不是感覺：每則仍走完確定性閘 → 獨立驗證者 → **發文前重驗**，任一關不過就不發。三道剎車：每日上限 8 則、篇距 90 秒、kill switch `~/.marketdaily-fallback/NEWSROOM_AUTOPOST_OFF`。手動單則模式保留（`--id ... --confirm`）。只發 Threads（每則 165-570 views vs IG 每則 3）。
  - ⭐ 手動與自動**共用同一個 `_prepare_for_post()`**（handle 遷移＋重驗）。不要為自動模式另寫寬鬆版：同一個不變量被兩種寫法各實作一次，就是便宜那層安靜死掉的起點。
...
看影片→`video-watcher`（禁只讀 transcript）｜logo 動態/片頭 sting/3D 字→`blender-logo-motion`（3D 空間不是 2D 描線，winrig 無頭 Cycles 0 元；AE 只做字卡合成）｜任何 After Effects 動畫建構/算圖(字卡/kinetic type/轉場/AE 段 logo/定版)→`ae-motion-pro`（09-24；無頭 aerender＋實測 `AP.*` 積木，交付前必過 `AP.qa` 閘門＋speed.png＋sheet 對參考）｜做廣告/宣傳片/品牌影片→`brand-ad-production`(AE 背景建構+算圖、Higgsfield 角色一致鏡頭、可商用旁白、多版本)｜任何影片音效/點擊聲/混音→`~/qfx/scripts/sfx_kit.py`（brand-ad-production §4；老闆禁配樂/鼓 bed/whoosh，樣本跨作品不重用，QA 不過不准交）｜做整站/landing→`website-design-team` 唯一入口（接案新站硬閘＝`impeccable` detect），**底子必從 ClaPat Bundle 按客戶品類選(2026-09-14 親令,SKILL ⓪e;Hayler 只給 qfxsolution.com),不手刻版面**｜合規審查→`legal-compliance` 唯一路徑，判「不過」不得繞道｜資安→`security-team` 總入口｜下注前→`quant-math` 必算｜上實盤前→`backtest-validation` 必驗｜生成式 prompt→`genai-prompt-pro` 前置｜全站巡檢→`site-doctor`｜去 AI 腔→`no-ai-slop`｜行銷鏈五連跑順序不可跳：spy→competitive-ads-extractor→bulk-creative→ads-score→ads-meta（週日 10:00 TW 自動，見 `marketing/CLAUDE.md`）｜edge 鏈：edge-pipeline-orchestrator→edge-signal-aggregator→edge-strategy-designer→signal-postmortem｜QFX 自家 IG/Threads 經營/發文/排內容→`qfx-social-ops`（queue+背景自動發；知名品牌 logo 片 09-24 起只從 QFX 帳號發，須 APPROVED 交接＋法務 lint）｜查個股信息差/籌碼/監理/內部人訊號→`intel-signal-lookup` 唯一路徑（要回「查無資料」或動用 `institutional-flow-tracker`/`invest-skill` 等付費 API 前**必先跑**，不手動 grep latest.json）
