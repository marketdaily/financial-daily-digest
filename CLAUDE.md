...
- **⭐ 發文路徑已接上（核可制）**：`python -m marketing.newsroom.run post` 不帶參數＝列待審清單；`--id <草稿id>` ＝預演；再加 `--confirm` 才真的發。**只發 Threads**（每則 165-442 views vs IG 每則 3；IG/FB 需圖卡未接）。⭐⭐ 發文前**重跑一次確定性閘**——這道防線當場抓到「草稿是在舊品牌名下產的、handle 已過期」，沒有它就會發出不存在的帳號名；草稿記 `brand_handle`，品牌變動時依該紀錄字面替換再重驗。
