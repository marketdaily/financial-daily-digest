...
看影片→`video-watcher`（禁只讀 transcript）｜logo 動態/片頭 sting/3D 字→`blender-logo-motion`（3D 空間不是 2D 描線，winrig 無頭 Cycles 0 元；AE 只做字卡合成）｜做廣告/宣傳片/品牌影片→`brand-ad-production`｜任何影片音效/點擊聲/混音→`~/qfx/scripts/sfx_kit.py`（brand-ad-production §4；老闆禁配樂/鼓 bed/whoosh，樣本跨作品不重用，QA 不過不准交）(AE 背景建構+算圖、Higgsfield 角色一致鏡頭、可商用旁白、多版本)｜做整站/landing→`website-design-team` 唯一入口（接案新站硬閘＝`impeccable` detect），**底子必從 ClaPat Bundle 按客戶品類選(2026-09-14 親令,SKILL ⓪e;Hayler 只給 qfxsolution.com),不手刻版面**｜合規審查→`legal-compliance` 唯一路徑，判「不過」不得繞道｜資安→`security-team` 總入口｜下注前→`quant-math` 必算｜上實盤前→`backtest-validation` 必驗｜生成式 prompt→`genai-prompt-pro` 前置｜全站巡檢→`site-doctor`｜去 AI 腔→`no-ai-slop`｜行銷鏈五連跑順序不可跳：spy→competitive-ads-extractor→bulk-creative→ads-score→ads-meta（週日 10:00 TW 自動，見 `marketing/CLAUDE.md`）｜edge 鏈：edge-pipeline-orchestrator→edge-signal-aggregator→edge-strategy-designer→signal-postmortem｜查個股信息差/籌碼/監理/內部人訊號→`intel-signal-lookup` 唯一路徑（要回「查無資料」或動用 `institutional-flow-tracker`/`invest-skill` 等付費 API 前**必先跑**，不手動 grep latest.json）
