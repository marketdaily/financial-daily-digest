...
def _pp_expand_tickers(html: str, tw_hint: dict) -> str:
    ...
    #   ② 前文緊鄰的是別支已知台股名 → 把代號**改正**成那支的代號。**名字說了算**,所以
    #      連「名稱表裡根本沒有的假代號」也會被改正 —— 舊版判準是「這 4 碼在表裡才剝」,
    #      剛好剝掉真代號、放行掰出來的假代號(9999),對它的立案動機完全失效。
    #   ③ 名字認不出來、而該碼是真台股代號 → 沿用舊行為剝掉(無從驗證的數字不放行)
    #   ④ 名字認不出來、也不是台股代號(年份/價位)→ 不碰;美股 ticker 是字母,本規則本來就不碰
