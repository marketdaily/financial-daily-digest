...
- **日報短影音產線（2026-07-08 上線）**：winrig cron `video_brief_runner.sh`（07:20–07:39 TW＋每日鎖）跑 `video_brief/run_daily.sh`：公版日報→`extract.py`（數字逐字回驗）→`render.py`（Playwright 錄 1080x1920 品牌動畫+程序配樂）→`make_post.py`（確定性驗證:數字回對/禁用詞/規格/非黑幀/非白幀→KV 上傳 `media.marketdaily.ai`→post json）。**內容鐵則：只上市場級重點＋事實性漲跌%，絕不上買賣價位/建議文字**（2026-06-02 明牌出包根因修正）；驗證不過=不產 post json=當天不發，生成失敗推 admin
