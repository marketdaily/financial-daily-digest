...
- **Reels 一律配最 trending 音樂（2026-07-10 用戶指令）**：`auto_post.py::post_instagram_reel` 自動查 IG Audio API `GET /ig_audio` 三層曲源（授權音樂 trending 榜 → 原創音訊 → Sound Collection `product=ADS`），掛 `audio_configuration` 並把影片內建配樂靜音；container/處理階段失敗 → fail-open 退回烘焙配樂重來（publish 只跑一次防重複發文），發文絕不因音樂缺席；曲庫首次解鎖自動推播 admin。**根因（2026-07-10 查明）：商業帳號依 Meta 授權協議拿不到授權音樂庫（app 內手動發也一樣）**，Delvin 拍板 A+B 都試：A=token 重發加 `ads_management`（解鎖 Sound Collection 推薦曲，合規乾淨）、B=IG 切 Creator 帳號（解鎖真 trending hits，灰色地帶：品牌內容用個人授權音樂最壞被消音/下架）。兩者任一完成後跑 `auto_post.py check` 看「IG 曲庫」行驗證。FB/TikTok/YT 的 API 無掛曲庫能力，維持烘焙配樂；**絕不把版權音樂直接烘進影片檔上傳**。有旁白的 reel 必須傳 `trending_audio=False`，否則旁白會被 `video_volume=0` 靜音
