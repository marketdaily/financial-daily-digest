...
check("A2 週一/週末 prompt 全部攔到(4 條)", len(PROMPTS) >= 4)
_with_skel = [p for p in PROMPTS
...
check("A3 每條週一/週末 prompt 都給逐字 TLDR 骨架(舊版白話寫法會 FAIL)",
      len(_with_skel) == len(PROMPTS))
check("A3b 舊版白話寫法已絕跡(不再只寫「.tldr 區改成…列 3-4 條」)",
      not any(re.search(r"\.tldr 區改成.{0,20}標題,列", p) for p in PROMPTS))
