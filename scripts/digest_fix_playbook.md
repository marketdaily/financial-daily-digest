...
3. **必加/更新一支回歸測試**(`scripts/test_*.py`)鎖住這個修復:證明「原本會誤判/出錯的輸入,修後正確;且真問題仍被抓」。
   測試有兩條硬規則(2026-08-17 實鍋,兩條都是那天違反的):
   - **它必須自己是綠的** —— guard 現在會實跑你新增的每一支 `scripts/test_*.py`,有一支紅整包丟棄。
     08-17 那次交出去的測試自己 2/19 紅,而當時 guard 只跑固定清單 ⇒ 一支從沒被執行過的裝飾測試進了生產樹。
   - **禁止打真實 API / 讀 `.env`** —— worktree 只含 tracked 檔,且自癒發生的時機正好是 LLM 配額最緊的時候。
