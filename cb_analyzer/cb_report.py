...
<details class="gloss"><summary>📐 分析原理與方法(用什麼算、怎麼估值、資料哪來——點開看完整說明)</summary>
  ...
  <div class="gitem"><b>10.5 兩種獲利口徑(為什麼有兩個機率)</b><br>
    · <b>死抱到期機率</b>:到期日股價高於回本點的機率(終值分佈)。這是保守下限——假設你買了就不動。<br>
    · <b>中途獲利機率(實戰口徑)</b>:存續期間內股價<b>曾經碰到</b>回本點的機率,用 GBM 反射原理解析解:
    <span style="color:#a5b4fc">P(touch) = N((−a+νT)/(σ√T)) + e^(2νa/σ²)·N((−a−νT)/(σ√T))</span>,a=ln(回本點/現價)、ν=μ−σ²/2。
    CBAS 下檔封頂,期間內碰到回本點就能帶著剩餘時間價值獲利出場——高波動深價外的標的,中途機率常是到期機率的 2~3 倍,
    這就是專業拆解玩家「一個組合放多注、贏在出場紀律」賺錢的數學原因。<br>
    · 另一個結構性因素:台灣 CB 發行方常有把股價推過轉換價的動機(轉股就不用還錢),
    真實轉換率高於隨機漫步假設 → 模型兩個機率都<b>偏保守</b>。</div>
