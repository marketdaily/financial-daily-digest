...
const I18N = {
  zh: {
    ...
    pos_title: "💼 我的部位", pos_unsigned: "永豐帳務 API 尚未生效(帳號回報 signed=false)。要兩步都完成才會解鎖:①到永豐「簽署中心」簽署 API 條款(證券／期貨需分開簽) ②完成模擬環境的 API 登入＋下單測試(週一至五 08:00–20:00)。兩步都做完後由永豐系統隔日過檔生效,不是即時。生效後這裡會自動顯示真實持股與未實現損益。",
  ...
  en: {
    ...
    pos_title: "💼 My Positions", pos_unsigned: "SinoPac account API access is not active yet (broker reports signed=false). Two steps are required: (1) sign the API terms in SinoPac's Sign Center (stock and futures are signed separately), and (2) complete the API login + order test in simulation mode (Mon–Fri 08:00–20:00). Activation happens on SinoPac's next-day batch, not immediately. Once active, real positions and unrealized P&L appear here.",
...
function rowHtml(sym) {
  ...
  return `<tr data-sym="${sym}"><td>${nameCell(sym)}</td>
    ...
    <td id="chg-${sym}" class="neutral">—</td>${bidask}${hlvol}</tr>`;
}
...
function renderRanks(rows) {
  ...
  document.querySelector("#rank-table tbody").innerHTML = rows.map((r, i) => {
    ...
    const pts = fmtPts(chgPts({ symbol: r.code, price: r.close, change: r.change_pct, change_val: r.change_price ?? null }));
